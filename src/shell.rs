...
use std::rc::Rc;
use crate::builtins::complete::CompleteCommand;
...
impl Shell {
    pub fn run(&self) {
        ...
        let complete_dict = Rc::new(RefCell::new(HashMap::<String, String>::new()));
        let builtins = Self::build_builtins(path_resolver, &complete_dict);
        let dispatcher = CommandDispatcher::new(builtins.0, external_commands);
        
        
        Self::run_loop(dispatcher, builtins.1, executable_cache, &complete_dict);
    }

    fn build_builtins(resolver: PathResolver, complete_dict: &Rc<RefCell<HashMap<String, String>>>) -> (HashMap<String, Box<dyn Command>>, HashSet<String>) {
        ...
        //complete cmd
        let complete_cmd = CompleteCommand::new("complete".to_string(), &complete_dict);
        builtins_list.insert(complete_cmd.get_name_copy());
        builtins_dict.insert(complete_cmd.get_name_copy(), Box::new(complete_cmd));
    ...
    fn run_loop(dispatcher: CommandDispatcher, builtin_commands: HashSet<String>, path_executables: HashSet<String>, completion_dict: &Rc<RefCell<HashMap<String, String>>>) {

        let config = Config::builder().completion_type(CompletionType::List).build();
        let mut readline = Editor::with_config(config).unwrap();
        readline.set_helper(Some(Completion::new(builtin_commands, path_executables, completion_dict)));
        loop {
